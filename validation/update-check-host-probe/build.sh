...
# Builds the task-local update-check host validation exerciser plugin JAR against
...
sdk_jar="${sdk_jars[0]}"
src="validation/update-check-host-probe/src"
...
javac --release 17 -cp "$sdk_jar" -d "$out" \
  "$src/dev/turboism/validation/updatecheck/UpdateCheckHostValidationPlugin.java"
cp -r "$src/META-INF" "$out/"

output="$repo_root/build/update-check-host-validation-exerciser.jar"
jar cf "$output" -C "$out" .
