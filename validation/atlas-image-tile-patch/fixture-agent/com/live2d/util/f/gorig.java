...
/**
 * Fixture replica of com.live2d.util.f.g — the private workaround method
 * implemented exactly as the 5303 bytecode (page-size scratch pipeline),
 * calling the fixture jp.noids helpers. Used as the transform target: the
 * patched body should be pixel-identical to this original.
 */
public final class gorig {

    public static final gorig a = new gorig();

    private gorig() {
    }

    /** Public entry mirroring how the kernel is reached — same branched
     *  structure as {@code g} so the two classes differ only in name. */
