...
# Builds the task-local animation-timeline host validation probe plugin JAR
# against the already-built SDK jar. The probe is validation tooling only;
...
base="validation/animation-timeline-host-probe"
src="$base/src"
out="$(mktemp -d)"
trap 'rm -rf "$out"' EXIT

javac --release 17 -cp "$sdk_jar" -d "$out" \
  "$src/dev/turboism/validation/animationtimeline/AnimationTimelineHostValidationPlugin.java"
cp -r "$src/META-INF" "$out/"

output="$repo_root/build/animation-timeline-host-validation-probe.jar"
jar cf "$output" -C "$out" .
