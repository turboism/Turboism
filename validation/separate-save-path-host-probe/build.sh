...
# Builds the task-local separate-save-path host validation probe plugin JAR
...
sdk_jar="${sdk_jars[0]}"
src="validation/separate-save-path-host-probe/src"
out="$(mktemp -d)"
trap 'rm -rf "$out"' EXIT

javac --release 17 -cp "$sdk_jar" -d "$out" \
  "$src/dev/turboism/validation/separatesavepath/SeparateSavePathHostValidationPlugin.java"
cp -r "$src/META-INF" "$out/"

output="$repo_root/build/separate-save-path-host-validation-probe.jar"
jar cf "$output" -C "$out" .
echo "[probe] $output"
sha256sum "$output"
