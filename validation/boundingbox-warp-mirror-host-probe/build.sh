...
# Builds the task-local BoundingBox warp-mirror reconnaissance probe against the
# already-built SDK jar, runs the offline direction-verifier self-check against the
...
base="validation/boundingbox-warp-mirror-host-probe"
plugin_pkg="dev/turboism/validation/boundingboxwarpmirror"
...
javac --release 17 -cp "$sdk_jar" -d "$out/plugin" \
  "$base/src/$plugin_pkg/BoundingBoxWarpMirrorHostValidationPlugin.java"
...
javac --release 17 -cp "$sdk_jar:$out/plugin" -d "$out/selfcheck" \
  "$base/selfcheck/$plugin_pkg/WarpMirrorSelfCheck.java"
java -cp "$sdk_jar:$out/plugin:$out/selfcheck" \
  dev.turboism.validation.boundingboxwarpmirror.WarpMirrorSelfCheck

# 3. Stage the plugin metadata and assemble the validation-only JAR.
cp -r "$base/src/META-INF" "$out/plugin/"
output="$repo_root/build/boundingbox-warp-mirror-host-validation-probe.jar"
...
for entry in \
  ...
  "$catalog" \
  "$plugin_pkg/BoundingBoxWarpMirrorHostValidationPlugin.class"
do
...
if jar tf "$output" | grep -Fq 'WarpMirrorSelfCheck'; then
