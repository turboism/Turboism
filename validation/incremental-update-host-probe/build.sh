...
# Builds the task-local incremental-update host validation exerciser plugin JAR
...
sdk_jar="${sdk_jars[0]}"
src="validation/incremental-update-host-probe/src"
out="$(mktemp -d)"
trap 'rm -rf "$out"' EXIT

javac --release 17 -cp "$sdk_jar" -d "$out" \
  "$src/dev/turboism/validation/incrementalupdate/IncrementalUpdateHostProbePlugin.java"
cp -r "$src/META-INF" "$out/"

output="$repo_root/build/incremental-update-host-validation-exerciser.jar"
jar cf "$output" -C "$out" .
echo "[probe] $output"
sha256sum "$output"
