...
LangString ManagedGraalHelp ${LANG_KOREAN} "고정된 아카이브를 GraalVM 공식 GitHub 릴리스에서 다운로드한 후 크기, SHA-256, 릴리스 메타데이터 및 격리 호스트를 검증한 다음 전용 영역에서 활성화합니다. 선택하지 않으면 다운로드하지 않습니다."
...
LangString ManagedGraalInstallError ${LANG_KOREAN} "관리형 GraalVM 설치에 실패했습니다. $INSTDIR\logs\installer\managed-graal-install.log를 확인하고, GitHub 다운로드가 가능하며 지원되는 Cubism Editor 또는 Java 17 이상이 있는지 확인한 후 다시 시도하세요."
