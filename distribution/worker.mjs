...
export class ReleaseRegistry {
 ...
 async fetch(request){
  ...
  if(stats){
   ...
   if(!this.snapshot||this.snapshot.error||Date.now()-Date.parse(this.snapshot.syncedAt)>30*60*1000)return jsonResponse(request,{status:'unavailable',error:{code:'STATS_UNAVAILABLE'}},503);
