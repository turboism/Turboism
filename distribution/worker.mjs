import { CHANNELS,ORIGIN,documentFor,jsonResponse } from './protocol.mjs';
...
export default {
 async fetch(request,env){
  ...
  if(path==='/health'||/^\/v1\/releases\/(stable|beta|nightly)\.json$/.test(path)||/^\/files\/[a-f0-9]{64}\/[A-Za-z0-9][A-Za-z0-9._-]*$/.test(path))return registry(env).fetch(request);
...
export class ReleaseRegistry {
 ...
 async refresh(){
  ...
  this.refreshing=(async()=>{
   try{this.snapshot=await synchronize(this.snapshot,this.env);this.save();}
   catch(error){this.snapshot={...(this.snapshot??{}),error:'SYNC_UNAVAILABLE'};this.save();console.error('Release refresh failed:',error.message);}
  })().finally(()=>{this.refreshing=null;});return this.refreshing;
 }
 async fetch(request){
  await this.ready;const path=new URL(request.url).pathname;
  if(path==='/_refresh'||!this.snapshot||this.snapshot.error||Date.now()-Date.parse(this.snapshot.syncedAt)>15*60*1000)await this.refresh();
  if(path==='/_refresh'||path==='/health'){
   const ok=Boolean(this.snapshot?.syncedAt&&!this.snapshot.error);return jsonResponse(request,{service:'turboism-release-api',status:ok?'ready':'unavailable',synchronizedAt:this.snapshot?.syncedAt??null,mirrorConfigured:Boolean(this.env.DOWNLOADS),channels:CHANNELS.map(channel=>({channel,status:documentFor(this.snapshot,channel).status}))},ok?200:503);
  }
  ...
  if(path.startsWith('/files/')){
   if(!this.snapshot||this.snapshot.error||Date.now()-Date.parse(this.snapshot.syncedAt)>30*60*1000)return jsonResponse(request,{error:{code:'SYNC_UNAVAILABLE'}},503);
   const key=path.slice(1);
   for(const value of Object.values(this.snapshot.known??{}))if(value.active){
    const a=value.release.assets.flatMap(x=>[x,x.checksum]).find(x=>x.key===key);if(a)return serveFile(request,this.env.DOWNLOADS,a);
   }
