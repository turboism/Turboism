import { hasVerifiedBytes } from './mirror.mjs';
...
export async function serveFile(request,bucket,asset){
 ...
 const head=await bucket.head(asset.key);if(!hasVerifiedBytes(head,asset))return new Response('File not mirrored',{status:503,headers:{'Cache-Control':'no-store'}});
 const etag='"'+asset.sha256+'"',headers=new Headers({'Content-Type':asset.mediaType,'Content-Disposition':`attachment; filename="${asset.name}"`,'Content-Length':String(asset.size),'Accept-Ranges':'bytes',ETag:etag,'X-Content-Type-Options':'nosniff','Access-Control-Allow-Origin':'*','Access-Control-Expose-Headers':'ETag, Content-Length, Content-Range, Accept-Ranges','Cache-Control':'private, no-store'});
