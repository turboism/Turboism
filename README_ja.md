...
インストール前に[エンドユーザー実行声明・免責事項](EULA.md)をお読みください。この声明は AGPL-3.0-only が付与する権利を制限するものではなく、簡体字中国語の正式文書が優先されます。本ソフトウェアは**現状のまま**提供されます。プロジェクトの内容を変更するプラグインや自動化機能を使う前に、独立したバックアップを保存してください。
