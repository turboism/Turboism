...
/** Runtime-owned typed-config I/O lane with plugin-executor completion. */
final class TypedConfigIoExecutor implements AutoCloseable {
    ...
    <T> CompletionStage<T> failed(final Throwable failure) {
        final PluginCompletionFuture<T> completion = future();
        dispatch(() -> completion.settleExceptionally(failure));
        return completion.stage();
    }
    ...
    public void close() {
        ...
        try {
            if (!executor.awaitTermination(CLOSE_TIMEOUT_SECONDS, TimeUnit.SECONDS)) {
                throw new IllegalStateException("Typed config I/O did not quiesce before scope close");
            }
    ...
    private final class Operation<T> implements Runnable {
        ...
        private final Supplier<T> unavailable;
