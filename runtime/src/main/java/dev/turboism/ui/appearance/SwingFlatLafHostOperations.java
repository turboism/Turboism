    ...
    public void refresh() {
        onEdt(() -> {
            try {
                ...
                // so updateUI keeps derived keys (e.g. CubismCommon.gl.viewArea.background
                // = darken(@background,8%)) overridden instead of recomputing them.
                flatLaf.getMethod("clearCustomDefaultsSource").invoke(null);
    ...
    private void writeCustomDefaultsSource(final Map<String, String> defaults) {
        try {
            ...
            for (Map.Entry<String, String> entry : defaults.entrySet()) {
                content.append(escapeKey(entry.getKey())).append('=').append(entry.getValue()).append('\n');
            }
