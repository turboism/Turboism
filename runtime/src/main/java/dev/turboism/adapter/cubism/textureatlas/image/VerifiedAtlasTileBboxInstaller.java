...
/**
 * Premain-only admission and lifecycle for the atlas tile-bbox scratch shrink.
 *
 * <p>Declines rather than fails when the target has already been loaded — transformation would
 * come too late — and treats an error while enumerating loaded classes as "already loaded", so
 * the fix is never installed on uncertain ground. Closing the installation removes the
 * transformer, which leaves the host class exactly as it was defined:</p>
 *
 * <p>Removal cannot undo a transform that has already been applied to a defined class, and the
 * installation handle says so through {@link Status}. Callers decide whether to keep the fix for
 * the process lifetime or to start without it.</p>
 */
public final class VerifiedAtlasTileBboxInstaller {

    private static final java.util.Set<String> TARGET_CLASS_NAMES = targetClassNames();

    private VerifiedAtlasTileBboxInstaller() {
    }
    ...
    public static final class Installation implements AutoCloseable {
        ...
        private final AtlasTileBboxTransformer transformer;
        ...
        public AtlasTileBboxTransformer.Outcome transformOutcome() {
            return transformer == null
                ? AtlasTileBboxTransformer.Outcome.NONE
                : transformer.outcome();
        }
    ...
    public static Installation install(final Instrumentation instrumentation,
        ...
        if (targetAlreadyLoaded(instrumentation)) {
            report(diagnostic, "ATLAS_TILE_BBOX_TARGET_ALREADY_LOADED");
        ...
        try {
            ...
            report(diagnostic, "ATLAS_TILE_BBOX_INSTALLED");
            return Installation.installed(instrumentation, transformer);
        } catch (RuntimeException failure) {
            report(diagnostic, "ATLAS_TILE_BBOX_INSTALL_FAILED");
    ...
    private static java.util.Set<String> targetClassNames() {
        ...
        for (final AtlasTileBboxTarget target : AtlasTileBboxTarget.REVIEWED) {
