 ...
 * panel, settings, logs, About, plugin management, and update hints. It is runtime-owned
 * framework code, not a plugin; the plugin-facing {@link PluginContext} service bundle is
 * assembled for it by the runtime so the shell keeps consuming the same contribution
 * surfaces plugins use.
 */
public final class CoreShell implements AutoCloseable {

    private PluginContext context;
    private PluginLogger logger;
    private MainToolbarHomeEntryService homeEntryService;
    private final ShellServices services;
    ...
    public CoreShell(final ShellServices services) {
        this.services = java.util.Objects.requireNonNull(services, "services");
    }

    /** Wires the shell's services, then contributes every built-in surface. */
