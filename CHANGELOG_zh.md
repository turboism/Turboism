...
- 在编辑器已回写过 config.json 的安装上升级时，不再出现“无法安全校验或更新 config.json”而中止：
  安装器侧迁移 schema 现在接受运行时写入的 textureAtlas 段（镜像 Java 运行时校验器，含字段类型），
  并新增可执行的迁移回归测试覆盖运行时段落、未知字段失败关闭与错误类型。
- 当上一次 Turboism 安装残留了托管快捷方式但状态文件缺失时（例如重装前删除了旧安装目录），
  安装器启用快捷方式不再失败：托管快捷方式路径由 Turboism 命名空间所有（托管开始菜单目录内
  的确定性命名），残留的常规文件直接替换，仅非常规条目保持失败关闭的拒绝行为。
