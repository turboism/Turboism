...
grep -Fq 'local-only' "$tmp/conflict.out" \
...
latest_runtime_log() { return 0; }
write_lifecycle_result() { printf 'lifecycle:%s:%s\n' "$1" "$2" > "$task_dir/lifecycle-test"; }
result_file=''
...
z_path() { printf '%s' "$1"; }
safe_label() { printf '%s' "$1"; }
local_prepare_directory() { transport_prepare_directory "$@"; }
local_copy_to() { transport_copy_to "$@"; }
local_copy_from() { transport_copy_from "$@"; }
launched=1
success=0
...
grep -Fq 'TASK CLEANUP FAILED' "$tmp/cleanup.out" \
...
execution_marker="$task_dir/hook.sh.executed"
mkdir -p "$task_dir"
printf '#!/bin/sh\nprintf "executed\\n" >> "$0.executed"\n' > "$remote_pre_cleanup"
local_copy_to() { printf 'copy\n' >> "$copy_marker"; return 23; }
...
rm -f "$copy_marker"
local_copy_to() { printf 'copy-success\n' >> "$copy_marker"; cp -- "$1" "$2"; }
remote_args_bash() { printf 'executed\n' >> "$execution_marker"; return 0; }
...
printf 'keep\n' > "$prefix_dir/keep"
transport_bash() { return 0; }
local_copy_from() { return 0; }
remote_stop_process_tree() { printf 'stopped\n' >> "$order"; return 0; }
keep_prefix=0
local_remove_tree() { return 0; }
case "$mode" in
  success)
    launched=0
    success=1
    mkdir -p "$local_evidence_dir"
    local_remove_tree() { printf 'removed\n' >> "$order"; rm -rf "$prefix_dir"; return 0; }
    original_status=0
    ;;
  evidence-failure)
    launched=0
...
grep -Fq 'TASK CLEANUP FAILED' "$tmp/exit-evidence.out" \
...
grep -Fq 'TASK CLEANUP FAILED' "$tmp/exit-prefix.out" \
...
# Preserve history's pre-launch idle guard without using the real process table.
...
# The legacy history spelling selects the same protected native transport.
TURBOISM_ENV_FILE=/dev/null bash "$runner" --execution-mode local \
  --name history-alias --version 5302 --bundle-root "$bundle" \
  --agent "$bundle/agent.jar" --plugin "$bundle/plugin.jar" \
  --fixture-local "$tmp/fixture.cmo3" --result-marker '{"status":"PASS"}' \
  --golden-prefix "$tmp/golden" --remote-root "$tmp/alias-tasks" \
  --proton-runner "$tmp/proton" --dry-run > "$tmp/history-alias.out"
grep -Fxq 'transport=local' "$tmp/history-alias.out" || fail history-alias
[ ! -e "$tmp/alias-tasks" ] || fail dry-run-side-effects
[ ! -e "$tmp/network-used" ] || fail history-alias-network
