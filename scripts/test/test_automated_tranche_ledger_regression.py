...
def main() -> None:
    ...
    for phase_id in sorted(module.PHASE0_FUTURE_WORK):
    ...
    expect_failure("phase0 future ceiling", lambda rows: row(rows, "automation.phase6.closure").update(readinessCeiling="AUTOMATED_TRANCHE_CLOSED", evidenceLevel="VERIFIED_STATIC_SYNTHETIC"), "future phase must remain NOT_STARTED/NONE/NONE during Phase 0")
    expect_failure("phase0 future status", lambda rows: row(rows, "phase1.ownership-audit").update(workStatus="PENDING"), "expected BOUNDED_SLICE/AUTO_NOW/NOT_STARTED")
    expect_failure("phase0 future evidence", lambda rows: row(rows, "automation.phase2.dispatcher-contract").update(evidenceLevel="PLAN"), "future phase must remain NOT_STARTED/NONE/NONE during Phase 0")
    ...
    expect_failure("premature tranche closed", lambda rows: row(rows, "tranche.automation.overall").update(workStatus="COMPLETE", evidenceLevel="VERIFIED_STATIC_SYNTHETIC", readinessCeiling="AUTOMATED_TRANCHE_CLOSED"), "expected sentinel tuple PENDING/PLAN/LEDGER_CORRECTED")
    expect_failure("tranche readiness escalation", lambda rows: row(rows, "tranche.automation.overall").update(readinessCeiling="OWNERSHIP_AUDITED"), "expected sentinel tuple PENDING/PLAN/LEDGER_CORRECTED")
