...
env -i PATH="$PATH" HOME="$HOME" TURBOISM_ENV_FILE="$tmp/missing.env" \
  bash -euc 'source "$1"; bash -euc '\''source "$1"; test -z "$TURBOISM_HOST_VALIDATION_SSH_HOST"; test -z "$TURBOISM_HOST_VALIDATION_SSH_KEY"; test -n "$TURBOISM_HOST_VALIDATION_FIXTURE_5302_SHA256"'\'' _ "$1"' _ "$loader"
