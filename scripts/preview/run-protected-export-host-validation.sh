...
case "$phase" in
  dialog | copy-binding | flatten | export | export-native | census \
    | atlas-fixture | census,atlas-fixture | expect-reject \
    | dialog,copy-binding \
    | copy-binding,flatten | dialog,copy-binding,flatten) ;;
  *)
    echo "error: unknown probe phase '$phase' (expected dialog, copy-binding, flatten, export, export-native, census, atlas-fixture, expect-reject, or a comma combination)" >&2
    exit 2
    ;;
esac
