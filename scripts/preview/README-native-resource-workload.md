...
After fresh host-idle inventory, remove `--dry-run` for a real run. Only official BAT launch in a task CoW prefix/home/model copy is allowed. Use a fresh short label per run. `profile` instead of `plain` adds bounded JFR profile recording (128MiB); these are diagnostic cohorts, not on/off optimization comparisons. Profiler overhead must be reported. No unrelated process cleanup.
