...
Common options:
  ...
  --ssh-host <user@host, default <validation-user>@<validation-host>>
  --ssh-key <path>
  --golden-prefix <host path, default <local-home>/.proton>
  --remote-root <host path, default <local-home>/TurboismValidation>
...
proton_runner='<local-home>/.local/share/Steam/compatibilitytools.d/GE-Proton10-34/proton'
keep_prefix=0
dry_run=0
...
if [ "$dry_run" = 1 ]; then
  ...
  for index in "${!jvm_options[@]}"; do
    ...
    printf 'jvmOption.%s.quoted=%s\n' "$index" "$(windows_java_tool_option "${jvm_options[$index]}")"
  done
  exit 0
fi
