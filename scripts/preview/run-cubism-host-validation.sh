...
Common options:
  ...
  --execution-mode <local|ssh>              compatibility alias for --transport
...
exit_timeout=120
poll_seconds=3
execution_mode="${TURBOISM_HOST_VALIDATION_EXECUTION_MODE:-}"
...
# Compatibility spelling for history wrappers; use the shared transport lifecycle.
...
if [ "$dry_run" = 1 ]; then
  printf '%s\n' \
    "name=$name" \
    "executionMode=$([ "$transport" = local ] && printf local || printf ssh)" \
    "transport=$transport" \
    "version=$version" \
...
if [ "$transport" = local ]; then host_validation_local_idle; fi
