...
for required in "$agent_jar" "$mesh_enhance_jar" "$test_classes/$probe_class_rel/WindowsMeshMirrorAxisValidationProbe.class" "$probe_descriptor"; do
  if [ ! -f "$required" ]; then
...
printf '  plugins/mesh-edit-mirror-axis-enhance.jar            %s bytes\n' "$(stat -c %s "$bundle_root/plugins/mesh-edit-mirror-axis-enhance.jar")"
