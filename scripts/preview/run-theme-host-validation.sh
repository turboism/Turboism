...
case "$version" in
  5302)
    ...
    cubism_unix='/home/local-user/.proton/pfx/drive_c/Program Files/Live2D Cubism 5.3'
  ...
  5203)
    ...
    cubism_unix='/home/local-user/.proton/pfx/drive_c/Program Files/Live2D Cubism 5.2'
...
task_dir="/home/local-user/TurboismThemeValidation/$version-$run_label/$task_id"
...
set -euo pipefail
task="$1"
cp -a --reflink=always /home/local-user/.proton "$task/prefix"
...
export DISPLAY=:0
cd "$task_dir" || exit 1
exec shorin-proton-wrapper -p "$task_dir/prefix" --runner /home/local-user/.local/share/Steam/compatibilitytools.d/GE-Proton10-34/proton --debug "$task_dir/prefix/pfx/drive_c/windows/system32/cmd.exe" /c "Z:$(winslash "$task_dir/launch.bat")" > "$task_dir/evidence/launcher.out" 2>&1
SH
