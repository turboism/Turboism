...
- **Windows distribution:** fx v0.0.5 has no reviewed Windows executable. Windows ZIP/NSIS Full nevertheless contains the exact full plugin roster, including `turboism-with-fx.jar`, but contains no `runtimes/fx/**`. Java Full rejects Windows before config or payload mutation. Java Thin can install the complete plugin roster with no native fx bytes, and Settings can accept an explicit user-owned custom executable path, but that is structural admission only: functional operation is unavailable because the required first-party MCP server fails closed until a native, handle-bound protected-DACL and reparse-safe bearer-file publication primitive exists. Lite remains plugin-free. A phase-1 compile-only v0.0.5 backport and other investigated upstream Windows ports remain validation research, not product runtimes, until their security, durability, ACP, exact-host, and process-tree gates are satisfied.
...
1. On reviewed Linux/macOS pairs, use Java Full to install the matching fx runtime and notices under `runtimes/fx/0.0.5/<platform>/`. On Windows, ZIP/NSIS Full includes the plugin but no managed fx executable, and Java Thin installs the plugin roster without native fx bytes. A custom executable path can be saved, but functional operation remains unavailable while first-party MCP bearer publication fails closed. Lite does not install this plugin.
...
| Windows has no fx executable | This is expected: Windows ZIP/NSIS Full includes the plugin but no managed fx runtime, and Java Thin can install it without native fx bytes. An explicit absolute custom path can be saved, but the plugin cannot operate because first-party MCP startup fails closed until protected-DACL and reparse-safe bearer publication exists. Turboism does not repackage Linux/macOS binaries or present validation bridges as product runtimes. |
