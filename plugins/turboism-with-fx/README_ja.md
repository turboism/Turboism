...
- **Windows distribution:** Windows NSIS Full, Full ZIP, and Java Full contain `runtimes/fx/0.0.5/windows-x86_64/fx.exe` with exact identity `11,144,192` bytes / SHA-256 `a36b0b209d933e4757d7e1a961d259d39a8d370b68cbde8e9cba227603ac63c2`. This is a Turboism build of upstream v0.0.5, not an official Vercel asset. It has no online repair archive; repair or reinstall Turboism Full to restore it. The Windows candidate admits only Turboism's exact authenticated numeric-loopback HTTP MCP server and keeps ACP sessions ephemeral; it does not claim durable-session, native-tool, general networking, process, or persistence parity with the official Linux/macOS runtimes.
...
|---|---|---|
| `turboism.action.register` | `application` | Agent ウィンドウと接続前の設定ウィンドウを開くアクションを登録します。 |
| `turboism.ui.menu.contribute` | `application` | Turboism メニューに直接 **fx 設定** 項目を追加します。 |
| `turboism.ui.toolbar.main.contribute` | `application` | Cubism のメインツールバーで Turboism Home の隣に fx Agent アイコンを追加します。 |
