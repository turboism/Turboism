...
以原子方式在名为 `manual-order-<scopeId>.txt` 的插件状态文件中写入版本头 `# turboism.scene.manual-order v1`，随后保存以换行分隔的项目 ID。无版本头的旧文件仍可读取；截断内容或未知未来格式会失败关闭。范围 ID 必须是一个不透明的 64 字符小写十六进制值；本插件不存储项目路径。
