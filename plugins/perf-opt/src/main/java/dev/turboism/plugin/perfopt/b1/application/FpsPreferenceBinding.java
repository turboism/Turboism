...
import dev.turboism.plugin.perfopt.b1.domain.FpsPresentationPreference;
...
public final class FpsPreferenceBinding {

    public static final String CONFIG_ID = "perf-opt.fps-presentation";
    public static final String CONFIG_PATH = "perf-opt/fps-presentation.cfg";
    private static final ConfigKey<Boolean> ENABLED = new ConfigKey<>(
        CONFIG_ID, "enabled", false, ConfigCodecs.booleanValue()
    );
    private static final ConfigSchema SCHEMA = new ConfigSchema(CONFIG_ID, CONFIG_PATH, 1, List.of(ENABLED));

    private PluginConfigRegistry registry;
    private FpsPresentationPreference confirmed = FpsPresentationPreference.defaults();
    ...
    public CompletionStage<ConfigBindingResult> enable() {
        ...
        return registry.read(ENABLED).handle((read, failure) -> {
            ...
            confirmed = FpsPresentationPreference.defaults().setEnabled(read.value().value());
            revision = read.value().revision();
            return ConfigBindingResult.APPLIED;
        });
    }

    public CompletionStage<ConfigBindingResult> setEnabled(final boolean value) {
        if (!enabled || registry == null) {
            return completed(ConfigBindingResult.DISABLED);
        }
        final FpsPresentationPreference next = confirmed.setEnabled(value);
        if (next.equals(confirmed)) {
            return completed(ConfigBindingResult.UNCHANGED);
        }
        final long activeEpoch = epoch;
        return registry.write(ENABLED, value, revision).handle((write, failure) -> {
            if (!enabled || epoch != activeEpoch) {
                return ConfigBindingResult.DISABLED;
            }
            if (failure != null) {
    ...
    public CompletionStage<ConfigBindingResult> toggle() {
        return setEnabled(!confirmed.enabled());
    }
    ...
    public FpsPresentationPreference confirmed() {
        return confirmed;
    }
