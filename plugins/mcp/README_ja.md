...
3 つの `*.apply` は引き続き `transactionEligible: false` であり、`turboism.transaction.execute` はトランザクション開始前に拒否します。個々のネイティブ操作に Undo がないという意味ではありません。`turboism.capabilities.read` で例外を確認できます。カバレッジ台帳はモデルオブジェクト、パラメーター、バインディングを含む明示された 12 型を対象とし、SDK 全体の網羅を保証しません。`MCP_LEGACY_WRITE` は公開済みの旧式書き込みを示し、グループ化トランザクションの検証完了を意味しません。追加行の空の `supportedVersions` は台帳が厳密なバージョン保証を行わないことを示し、実行時には Provider の准入判定が適用されます。
