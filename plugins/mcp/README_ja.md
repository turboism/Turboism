...
- **Turboism API:** `[0.1.0,0.2.0)`。
- **Cubism:** Turboism は現在、レビュー済みの正確な Editor アーティファクト `5.2.03` と `5.3.02` を受け入れます。ホスト向けリソースは、その公開 SDK 機能を利用できないとき、クローズドに失敗します。空の成功結果を捏造することはありません。
...
2. 所有者専用権限を強制できる POSIX ファイルシステムでは、プラグイン状態ディレクトリから生成された `mcp-connection.json` を読み取ります。Windows では Java 17 が保護 DACL とハンドル相対のリパース安全な Bearer ファイル公開を証明できないため、現在は起動時に fail closed します。
...
接続ファイルにはローカルシークレットが含まれます。ログに記録したり、検証証跡にコピーしたり、信頼できないプロセスに公開したりしないでください。Windows 対応には、最初から保護されたディレクトリ／ファイルハンドルを作成して保持し、保護 DACL、リパースポイントとファイル ID の検証、ハンドル経由の書き込み／フラッシュ、アトミック公開を行うランタイム所有のネイティブなプライベート状態階層／公開サービスが必要です。`Files.createDirectories` が継承 ACL で作成した後に権限を狭めても、既に開かれたハンドルによるアクセスは取り消せないため、プラグインローカルの一回限りのスクリプトや後続 ACL 書き換えでは不十分です。
