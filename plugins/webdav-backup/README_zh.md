...
|---|---|
| 版本 | `0.1.0` |
| 插件 ID | `dev.turboism.plugin.webdav` |
| 类别 | `workflow` |
...
| 升级后出现两个 WebDAV 条目 | 插件已改名为 `plugins/webdav-backup.jar`；托管升级会按内嵌插件 id 删除改名前的 `backup.jar`，旧 id `dev.turboism.plugin.backup` 会被运行时拒绝。只有在安装器无法删除被占用的文件时才需要手动删除残留的 `backup.jar`。 |
